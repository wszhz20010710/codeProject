...
def log_filter(img, K_size = 3, sigma = 3):
    ...
    for x in range(K_size):
        for y in range(K_size):
            K[x, y] = ((x ** 2 + y ** 2 - sigma ** 2) * np.exp((-x ** 2 - y ** 2) / (sigma ** 2 * 2))) / (sigma ** 6 * 2 * np.pi)
    K /= K.sum()
...
gray = BGR2GRAY(pic)
# filtering
result = log_filter(gray, K_size=3, sigma= 3)
# show result
cv2.imshow('result',result)
# save result
cv2.imwrite("D:\\codeProject\\Python\\ImageProcessing100Wen\\Question11-20\\answer19.jpg",result)
cv2.waitKey(0)
